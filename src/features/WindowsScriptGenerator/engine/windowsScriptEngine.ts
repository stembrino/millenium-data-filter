...
export const generateWindowsMoveScript = (
  ...
  const content = [
    ...
    "        $duplicatePattern = \"^\" + [regex]::Escape($baseName) + \"\\.*\" + [regex]::Escape($extension) + \"$\"",
    ...
    '            Write-Host "AVISO: O arquivo $baseName esta repetido $($matchingFiles.Count) vezes, nao sera movido." -ForegroundColor Yellow',
    "            $duplicateCount++",
    "            continue",
    "        }",
    ...
    "}",
    "",
    "if (($missingCount + $failedCount + $duplicateCount) -gt 0) {",
    "    exit 1",
    "}",
    "exit 0",
  ].join("\n");
